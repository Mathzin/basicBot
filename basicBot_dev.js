    ...
    var esBot = {
        ...
        room: {
            ...
            roulette: {
                ...
                startRoulette: function () {
                    ...
                    esBot.room.roulette.countdown = setTimeout(function () {
                        esBot.room.roulette.endRoulette();
                    }, 60 * 1000);
        ...
        userUtilities: {
            ...
            getPermission: function (obj) { //1 requests
                ...
                if(esBot.creatorIDs.indexOf(u.id) > -1) return 10;
                ...
                for (var i = 0; i < lastPos; i++) {
                    if (typeof djlist[i] !== 'undefined') {
                        ...
                        if (typeof user !== 'boolean') {
                            ...
                            if (rank !== null && esBot.userUtilities.getPermission(plugUser) <= rank) {
        ...
        eventChat: function (chat) {
            for (var i = 0; i < esBot.room.users.length; i++) {
                if (esBot.room.users[i].id === chat.uid) {
                    esBot.userUtilities.setLastActivity(esBot.room.users[i]);
                    if (esBot.room.users[i].username !== chat.un) {
                        esBot.room.users[i].username = chat.un;
                    }
                }
            }
        ...
        eventDjadvance: function (obj) {
            ...
            if (esBot.room.autoskip) {
                ...
                esBot.room.autoskipTimer = setTimeout(function () {
                    API.moderateForceSkip();
                }, remaining - 500);
            }
            storeToStorage();
        ...
        chatcleaner: function (chat) {
            ...
            if (esBot.userUtilities.getPermission(chat.uid) > 1) return false;
            ...
            if (capitals >= 40) {
                API.sendChat("/me @" + chat.un + ", unglue your capslock button please.");
                return true;
            }
            msg = msg.toLowerCase();
            if (msg === 'skip') {
                API.sendChat("/me @" + chat.un + ", don't ask for skips.");
                return true;
            }
            for (var j = 0; j < esBot.chatUtilities.spam.length; j++) {
                if (msg === esBot.chatUtilities.spam[j]) {
                    API.sendChat("/me @" + chat.un + ", please don't spam.");
                    return true;
                }
            }
            /**
            for (var i = 0; i < esBot.chatUtilities.beggarSentences.length; i++) {
                if (msg.indexOf(esBot.chatUtilities.beggarSentences[i]) >= 0) {
                    API.sendChat("/me @" + chat.un + ", please don't ask for fans.");
                    return true;
                }
            }
            **/
        ...
        chatUtilities: {
            chatFilter: function (chat) {
                var msg = chat.message;
                var perm = esBot.userUtilities.getPermission(chat.uid);
                var user = esBot.userUtilities.lookupUser(chat.uid);
                var isMuted = false;
                for (var i = 0; i < esBot.room.mutedUsers.length; i++) {
                    if (esBot.room.mutedUsers[i] === chat.uid) isMuted = true;
                }
                if (isMuted) {
                ...
                if (plugRoomLinkPatt.exec(msg)) {
                    ...
                    if (perm === 0) {
                        API.sendChat("/me @" + chat.un + ", don't post links to other rooms please.");
                        API.moderateDeleteChat(chat.cid);
                        return true;
                    }
                }
                if (msg.indexOf('http://adf.ly/') > -1) {
                    API.moderateDeleteChat(chat.cid);
                    API.sendChat('/me @' + chat.un + ', please change your autowoot program. We suggest PlugCubed: http://plugcubed.net/');
                    return true;
                }
                ...
                if ((msg.indexOf("joined the roulette") > -1 || msg.indexOf("left the roulette") > -1) && chat.uid === esBot.loggedInID) {
            ...
            commandCheck: function (chat) {
                ...
                var userPerm = esBot.userUtilities.getPermission(chat.uid);
                ...
                if (chat.message === '!eta' && esBot.roomSettings.etaRestriction) {
                    if (userPerm < 2) {
                        var u = esBot.userUtilities.lookupUser(chat.uid);
                ...
                for (var comm in esBot.commands) {
                    var cmdCall = comm.command;
                    if(!Array.isArray(comm.command)) cmdCall = [cmdCall];
                    for(var ind in cmdCall){
                        if (esBot.roomSettings.commandLiteral + cmdCall[ind] === cmd) {
                            comm.functionality(chat, esBot.roomSettings.commandLiteral + cmdCall[ind]);
                            executed = true;
                            break;
                        }
                    }
                ...
                if (executed && userPerm === 0) {
                    ...
                    setTimeout(function () {
                        esBot.room.usercommand = true;
                    }, esBot.roomSettings.commandCooldown * 1000);
                }
                if (executed) {
                    API.moderateDeleteChat(chat.cid);
                    esBot.room.allcommand = false;
                    setTimeout(function () {
                        esBot.room.allcommand = true;
                    }, 5 * 1000);
                }
                return executed;
            },
            action: function (chat) {
                ...
                if (chat.type === 'message') {
                    for (var j = 0; j < esBot.room.users.length; j++) {
                        if (esBot.userUtilities.getUser(esBot.room.users[j]).id === chat.uid) {
                            esBot.userUtilities.setLastActivity(esBot.room.users[j]);
                        }
        ...
        startup: function () {
            var u = API.getUser();
            if (esBot.userUtilities.getPermission(u) < 2) return API.chatLog("Only bouncers and up can run a bot.");
            if (esBot.userUtilities.getPermission(u) === 2) API.chatLog("The bot can't move people when it's run as a bouncer.");
            /**
            ...
            **/
            ...
            esBot.room.afkInterval = setInterval(function () {
                esBot.roomUtilities.afkCheck()
            }, 10 * 1000);
            esBot.room.autodisableInterval = setInterval(function () {
                esBot.room.autodisableFunc();
            }, 60 * 60 * 1000);
            esBot.loggedInID = API.getUser().id;
            esBot.status = true;
            API.sendChat('/cap 1');
            API.setVolume(0);
            API.sendChat('/me ' + esBot.roomSettings.botName + ' v' + esBot.version + ' online!');
            window.bot = esBot;
        },
        commands: {
                ...
                functionality: function (chat, cmd) {
                    ...
                    if (!esBot.commands.executable(this.rank, chat)) return void (0);
                        ...
                        if (msg.length === cmd.length) time = 60;
                            ...
                            if (isNaN(time)) return API.sendChat('/me [@' + chat.un + '] Invalid time specified.');
                        }
                        for (var i = 0; i < esBot.room.users.length; i++) {
                        ...
                        API.sendChat('/me [@' + chat.un + '] There have been ' + chatters + ' users chatting in the past ' + time + ' minutes.');
                    }
                }
            },
            ...
            afklimitCommand: {
                ...
                functionality: function (chat, cmd) {
                    ...
                    if (!esBot.commands.executable(this.rank, chat)) return void (0);
                        ...
                        var msg = chat.message;
                        if (msg.length === cmd.length) return API.sendChat('/me [@' + chat.un + '] No limit specified');
                        var limit = msg.substring(cmd.length + 1);
                        if (!isNaN(limit)) {
                            esBot.roomSettings.maximumAfk = parseInt(limit, 10);
                            API.sendChat('/me [@' + chat.un + '] Maximum afk duration set to ' + esBot.roomSettings.maximumAfk + ' minutes.');
                        }
                        else API.sendChat('/me [@' + chat.un + '] Invalid limit.');
                    }
                }
            },
                ...
                functionality: function (chat, cmd) {
                    ...
                    if (!esBot.commands.executable(this.rank, chat)) return void (0);
                        ...
                        if (esBot.roomSettings.afkRemoval) {
                            ...
                            clearInterval(esBot.room.afkInterval);
                            API.sendChat('/me [@' + chat.un + '] Turned afk removal off.');
                        }
                        else {
                            esBot.roomSettings.afkRemoval = !esBot.roomSettings.afkRemoval;
                            esBot.room.afkInterval = setInterval(function () {
                            ...
                            API.sendChat('/me [@' + chat.un + '] Turned afk removal on.');
                        }
                    }
                }
            },
            ...
            afkresetCommand: {
                ...
                functionality: function (chat, cmd) {
                    ...
                    if (!esBot.commands.executable(this.rank, chat)) return void (0);
                        ...
                        API.sendChat('/me [@' + chat.un + '] Reset the afk status of @' + name + '.');
                    }
                }
            },
            ...
            afktimeCommand: {
                ...
                functionality: function (chat, cmd) {
                    ...
                    if (!esBot.commands.executable(this.rank, chat)) return void (0);
                        ...
                        var inactivity = Date.now() - lastActive;
                        var time = esBot.roomUtilities.msToStr(inactivity);
                        API.sendChat('/me [@' + chat.un + '] @' + name + ' has been inactive for ' + time + '.');
                    }
                }
            },
                ...
                functionality: function (chat, cmd) {
                    ...
                    if (!esBot.commands.executable(this.rank, chat)) return void (0);
                        ...
                        if (esBot.roomSettings.autoskip) {
                            ...
                            clearTimeout(esBot.room.autoskipTimer);
                            return API.sendChat('/me [@' + chat.un + '] Autoskip disabled.');
                        }
                        else {
                            esBot.roomSettings.autoskip = !esBot.roomSettings.autoskip;
                            return API.sendChat('/me [@' + chat.un + '] Autoskip enabled.');
                        }
                    }
                }
            },
            ...
            banCommand: {
                ...
                functionality: function (chat, cmd) {
                    ...
                    if (!esBot.commands.executable(this.rank, chat)) return void (0);
                        ...
                        var msg = chat.message;
                        if (msg.length === cmd.length) return API.sendChat('/me [@' + chat.un + '] No valid user specified.');
                        var name = msg.substr(cmd.length + 2);
                        var user = esBot.userUtilities.lookupUserName(name);
                        if (typeof user === 'boolean') return API.sendChat('/me [@' + chat.un + '] Invalid user specified.');
                ...
                functionality: function (chat, cmd) {
                    ...
                    if (!esBot.commands.executable(this.rank, chat)) return void (0);
                        ...
                        if (esBot.roomSettings.bouncerPlus) {
                            ...
                            return API.sendChat('/me [@' + chat.un + '] Bouncer+ is now off.');
                        }
                        else {
                            if (!esBot.roomSettings.bouncerPlus) {
                                var id = chat.uid;
                                ...
                                if (perm > 2) {
                                    ...
                                    return API.sendChat('/me [@' + chat.un + '] Bouncer+ is now on.');
                                }
                            }
                            else return API.sendChat('/me [@' + chat.un + '] You have to be manager or up to enable Bouncer+.');
                        }
                    }
                }
            },
            ...
            clearchatCommand: {
                ...
                functionality: function (chat, cmd) {
                    ...
                    if (!esBot.commands.executable(this.rank, chat)) return void (0);
                        ...
                        return API.sendChat('/me [@' + chat.un + '] Cleared the chat.');
                    }
                }
            },
            ...
            commandsCommand: {
                ...
                functionality: function (chat, cmd) {
                    ...
                    if (!esBot.commands.executable(this.rank, chat)) return void (0);
                        ...
                        API.sendChat("/me " + esBot.roomSettings.botName + " commands: " + esBot.cmdLink);
                    }
                }
            },
                    ...
                    if (!esBot.commands.executable(this.rank, chat)) return void (0);
                        ...
                        if (space === -1) {
                            ...
                            if (user === false || !user.inRoom) {
                                ...
                                return API.sendChat("/me @" + user.username + ", @" + chat.un + ' ' + this.getCookie());
                            }
                        }
                    }
                }
                ...
                functionality: function (chat, cmd) {
                    ...
                    if (!esBot.commands.executable(this.rank, chat)) return void (0);
                        ...
                        if (esBot.roomSettings.cycleGuard) {
                            ...
                            return API.sendChat('/me [@' + chat.un + '] Cycleguard disabled.');
                        }
                        else {
                            esBot.roomSettings.cycleGuard = !esBot.roomSettings.cycleGuard;
                            return API.sendChat('/me [@' + chat.un + '] Cycleguard enabled.');
                        }
                ...
                functionality: function (chat, cmd) {
                    ...
                    if (!esBot.commands.executable(this.rank, chat)) return void (0);
                        ...
                        if (!isNaN(cycleTime)) {
                            ...
                            return API.sendChat('/me [@' + chat.un + '] The cycleguard is set to ' + esBot.roomSettings.maximumCycletime + ' minute(s).');
                        }
                        else return API.sendChat('/me [@' + chat.un + '] No correct time specified for the cycleguard.');
                ...
                functionality: function (chat, cmd) {
                    ...
                    if (!esBot.commands.executable(this.rank, chat)) return void (0);
                        ...
                        if (msg.length === cmd.length) name = chat.un;
                            ...
                            var perm = esBot.userUtilities.getPermission(chat.uid);
                            if (perm < 2) return API.sendChat('/me [@' + chat.un + '] Only bouncers and above can do !dclookup for others.');
                        }
                        var user = esBot.userUtilities.lookupUserName(name);
                        if (typeof user === 'boolean') return API.sendChat('/me [@' + chat.un + '] Invalid user specified.');
                        var id = user.id;
                ...
                functionality: function (chat, cmd) {
                    ...
                    if (!esBot.commands.executable(this.rank, chat)) return void (0);
                        ...
                        if (typeof esBot.roomSettings.fbLink === "string")
                            API.sendChat('/me [' + chat.un + '] Like us on facebook: ' + esBot.roomSettings.fbLink);
                    }
                }
            },
                ...
                functionality: function (chat, cmd) {
                    ...
                    if (!esBot.commands.executable(this.rank, chat)) return void (0);
                        ...
                        if (esBot.roomSettings.filterChat) {
                            ...
                            return API.sendChat('/me [@' + chat.un + '] chatfilter disabled.');
                        }
                        else {
                            esBot.roomSettings.filterChat = !esBot.roomSettings.filterChat;
                            return API.sendChat('/me [@' + chat.un + '] chatfilter enabled.');
                        }
                    }
                }
            },
            ...
            joinCommand: {
                ...
                functionality: function (chat, cmd) {
                    ...
                    if (!esBot.commands.executable(this.rank, chat)) return void (0);
                        ...
                        if (esBot.room.roulette.rouletteStatus && esBot.room.roulette.participants.indexOf(chat.uid) < 0) {
                            esBot.room.roulette.participants.push(chat.uid);
                            API.sendChat("/me @" + chat.un + " joined the roulette! (!leave if you regret it.)");
                        }
                    }
                }
            },
            ...
            jointimeCommand: {
                ...
                functionality: function (chat, cmd) {
                    ...
                    if (!esBot.commands.executable(this.rank, chat)) return void (0);
                        ...
                        API.sendChat('/me [@' + chat.un + '] @' + name + ' has been in the room for ' + timeString + '.');
                    }
                }
            },
            ...
            kickCommand: {
                ...
                functionality: function (chat, cmd) {
                    ...
                    if (!esBot.commands.executable(this.rank, chat)) return void (0);
                        ...
                        var user = esBot.userUtilities.lookupUserName(name);
                        var from = chat.un;
                        if (typeof user === 'boolean') return API.sendChat('/me [@' + chat.un + '] No valid user specified.');

                        var permFrom = esBot.userUtilities.getPermission(chat.uid);
                        var permTokick = esBot.userUtilities.getPermission(user.id);

                        if (permFrom <= permTokick)
                            return API.sendChat("/me [@" + chat.un + "] you can't kick users with an equal or higher rank than you!")

                        if (!isNaN(time)) {
                            API.sendChat('/me [' + chat.un + ' used kick, it\'s super effective!]');
                        ...
                        else API.sendChat('/me [@' + chat.un + '] No valid time (minutes) specified.');
                    }
                }
            },
            ...
            leaveCommand: {
                ...
                functionality: function (chat, cmd) {
                    ...
                    if (!esBot.commands.executable(this.rank, chat)) return void (0);
                        ...
                        var ind = esBot.room.roulette.participants.indexOf(chat.uid);
                        if (ind > -1) {
                            esBot.room.roulette.participants.splice(ind, 1);
                            API.sendChat("/me @" + chat.un + " left the roulette!");
                        }
                    }
                }
            },
                        ...
                        if (perm >= 1 || isDj) {
                            ...
                            if (media.format === '2') {
                                ...
                                SC.get('/tracks/' + media.cid, function (sound) {
                                    API.sendChat('/me [' + from + '] Link to current song: ' + sound.permalink_url);
                                });
                            }
                        }
                    }
            ...
            lockdownCommand: {
                ...
                functionality: function (chat, cmd) {
                    ...
                    if (!esBot.commands.executable(this.rank, chat)) return void (0);
                        ...
                        if (esBot.roomSettings.lockdownEnabled) {
                            return API.sendChat("/me [@" + chat.un + "] Lockdown enabled. Only staff can chat now.");
                        }
                        else return API.sendChat('/me [@' + chat.un + '] Lockdown disabled.');
                    }
                }
            },
                ...
                functionality: function (chat, cmd) {
                    ...
                    if (!esBot.commands.executable(this.rank, chat)) return void (0);
                        ...
                        if (esBot.roomSettings.lockGuard) {
                            ...
                            return API.sendChat('/me [@' + chat.un + '] Lockguard disabled.');
                        }
                        else {
                            esBot.roomSettings.lockGuard = !esBot.roomSettings.lockGuard;
                            return API.sendChat('/me [@' + chat.un + '] Lockguard enabled.');
                        }
                    }
                }
            },
                    ...
                    if (!esBot.commands.executable(this.rank, chat)) return void (0);
                        ...
                        if (esBot.room.skippable) {
                            ...
                            if (chat.message.length === cmd.length) {
                                API.sendChat('/me [' + chat.un + ' used lockskip.]');
                                esBot.roomUtilities.booth.lockBooth();
                                //esBot.roomUtilities.changeDJCycle();
                                setTimeout(function (id) {
                                    API.moderateForceSkip();
                ...
                functionality: function (chat, cmd) {
                    ...
                    if (!esBot.commands.executable(this.rank, chat)) return void (0);
                        ...
                        if (!isNaN(pos)) {
                            ...
                            return API.sendChat('/me [@' + chat.un + '] Lockskip will now move the dj to position ' + esBot.roomSettings.lockskipPosition + '.');
                        }
                        else return API.sendChat('/me [@' + chat.un + '] No valid position specified.');
                    }
                }
            },
                ...
                functionality: function (chat, cmd) {
                    ...
                    if (!esBot.commands.executable(this.rank, chat)) return void (0);
                        ...
                        if (!isNaN(lockTime)) {
                            ...
                            return API.sendChat('/me [@' + chat.un + '] The lockguard is set to ' + esBot.roomSettings.maximumLocktime + ' minute(s).');
                        }
                        else return API.sendChat('/me [@' + chat.un + '] No correct time specified for the lockguard.');
                    }
                }
            },
                ...
                functionality: function (chat, cmd) {
                    ...
                    if (!esBot.commands.executable(this.rank, chat)) return void (0);
                        ...
                        if (!isNaN(maxTime)) {
                            ...
                            return API.sendChat('/me [@' + chat.un + '] The maximum song duration is set to ' + esBot.roomSettings.maximumSongLength + ' minutes.');
                        }
                        else return API.sendChat('/me [@' + chat.un + '] No correct duration specified.');
                    }
                }
            },
            ...
            moveCommand: {
                ...
                functionality: function (chat, cmd) {
                    ...
                    if (!esBot.commands.executable(this.rank, chat)) return void (0);
                        ...
                        var user = esBot.userUtilities.lookupUserName(name);
                        if (typeof user === 'boolean') return API.sendChat('/me [@' + chat.un + '] Invalid user specified.');
                        if (user.id === esBot.loggedInID) return API.sendChat('/me [@' + chat.un + '] Don\'t try to add me to the waitlist, please.');
                        if (!isNaN(pos)) {
                            API.sendChat('/me [' + chat.un + ' used move.]');
                            esBot.userUtilities.moveUser(user.id, pos, false);
                        } else return API.sendChat('/me [@' + chat.un + '] Invalid position specified.');
                    }
                }
            },
                ...
                functionality: function (chat, cmd) {
                    ...
                    if (!esBot.commands.executable(this.rank, chat)) return void (0);
                        ...
                        if (lastSpace === msg.indexOf(' ')) {
                            ...
                            time = msg.substring(lastSpace + 1);
                            if (isNaN(time)) {
                                return API.sendChat('/me [@' + chat.un + '] Invalid time specified.');
                            }
                            name = msg.substring(cmd.length + 2, lastSpace);
                        }
                        var from = chat.un;
                        ...
                        if (permFrom > permUser) {
                            ...
                            if (time === null) API.sendChat('/me [@' + chat.un + '] Muted @' + name + '.');
                            else {
                                API.sendChat('/me [@' + chat.un + '] Muted @' + name + ' for ' + time + ' minutes.');
                                setTimeout(function (id) {
                                    ...
                                    if (indexMuted > -1) {
                                        ...
                                        var name = u.username;
                                        API.sendChat('/me [@' + chat.un + '] Unmuted @' + name + '.');
                                    }
                                }, time * 60 * 1000, user.id);
                            }
                        }
                        else API.sendChat("/me [@" + chat.un + "] You can't mute persons with an equal or higher rank than you.");
                    }
                }
            },
                ...
                functionality: function (chat, cmd) {
                    ...
                    if (!esBot.commands.executable(this.rank, chat)) return void (0);
                        ...
                        if (msg.length > cmd.length + 2) {
                            ...
                            } else API.sendChat("/me [@" + chat.un + "] Specified user @" + name + " is not in the waitlist.");
                        } else API.sendChat("/me [@" + chat.un + "] No user specified.");
                    }
                }
            },
                ...
                functionality: function (chat, cmd) {
                    ...
                    if (!esBot.commands.executable(this.rank, chat)) return void (0);
                        ...
                        if (esBot.roomSettings.etaRestriction) {
                            ...
                            return API.sendChat('/me [@' + chat.un + '] eta unrestricted.');
                        }
                        else {
                            esBot.roomSettings.etaRestriction = !esBot.roomSettings.etaRestriction;
                            return API.sendChat('/me [@' + chat.un + '] eta restricted.');
                        }
                    }
                }
            },
            ...
            skipCommand: {
                ...
                functionality: function (chat, cmd) {
                    ...
                    if (!esBot.commands.executable(this.rank, chat)) return void (0);
                        ...
                        API.sendChat('/me [' + chat.un + ' used skip.]');
                        API.moderateForceSkip();
                        esBot.room.skippable = false;
                        setTimeout(function () {
                            esBot.room.skippable = true
                    ...
                    if (!esBot.commands.executable(this.rank, chat)) return void (0);
                        ...
                        if (space === -1) {
                            ...
                            if (user === false || !user.inRoom) {
                                ...
                                return API.sendChat("/me @" + user.username + ", @" + chat.un + ' ' + this.getSlap());
                            }
                        }
                    }
                }
            ...
            swapCommand: {
                ...
                functionality: function (chat, cmd) {
                    ...
                    if (!esBot.commands.executable(this.rank, chat)) return void (0);
                        ...
                        if (typeof user1 === 'boolean' || typeof user2 === 'boolean') return API.sendChat('/me [@' + chat.un + '] Invalid user specified. (No names with spaces!)');
                        if (user1.id === esBot.loggedInID || user2.id === esBot.loggedInID) return API.sendChat('/me [@' + chat.un + '] Don\'t try to add me to the waitlist, please.');
                        var p1 = API.getWaitListPosition(user1.id);
                        var p2 = API.getWaitListPosition(user2.id);
                        if (p1 < 0 || p2 < 0) return API.sendChat('/me [@' + chat.un + '] Please only swap users that are in the waitlist!');
                ...
                functionality: function (chat, cmd) {
                    ...
                    if (!esBot.commands.executable(this.rank, chat)) return void (0);
                        ...
                        if (esBot.roomSettings.timeGuard) {
                            ...
                            return API.sendChat('/me [@' + chat.un + '] Timeguard disabled.');
                        }
                        else {
                            esBot.roomSettings.timeGuard = !esBot.roomSettings.timeGuard;
                            return API.sendChat('/me [@' + chat.un + '] Timeguard enabled.');
                        }
                ...
                functionality: function (chat, cmd) {
                    ...
                    if (!esBot.commands.executable(this.rank, chat)) return void (0);
                        ...
                        setTimeout(function (chat) {
                            ...
                            if (!found) {
                                $(".icon-chat").click();
                                return API.sendChat('/me [@' + chat.un + '] The user was not banned.');
                            }
                    ...
                    if (!esBot.commands.executable(this.rank, chat)) return void (0);
                        ...
                        if (msg.indexOf('@') === -1 && msg.indexOf('all') !== -1) {
                            if (permFrom > 2) {
                                ...
                                return API.sendChat('/me [@' + chat.un + '] Unmuted everyone.');
                            }
                            else return API.sendChat('/me [@' + chat.un + '] Only managers and up can unmute everyone at once.')
                        }
                        ...
                        if (permFrom > permUser) {
                            ...
                            if (!wasMuted) return API.sendChat("/me [@" + chat.un + "] that user wasn't muted.");
                            esBot.room.mutedUsers.splice(indexMuted);
                            API.sendChat('/me [@' + chat.un + '] Unmuted @' + name + '.');
                        }
                        else API.sendChat("/me [@" + chat.un + "] You can't unmute persons with an equal or higher rank than you.");
                    }
                }
            },
                ...
                functionality: function (chat, cmd) {
                    ...
                    if (!esBot.commands.executable(this.rank, chat)) return void (0);
                        ...
                        if (!isNaN(cd)) {
                            ...
                            return API.sendChat('/me [@' + chat.un + '] The cooldown for commands by users is now set to ' + esBot.roomSettings.commandCooldown + ' seconds.');
                        }
                        else return API.sendChat('/me [@' + chat.un + '] No correct cooldown specified.');
                    }
                }
            },
                ...
                functionality: function (chat, cmd) {
                    ...
                    if (!esBot.commands.executable(this.rank, chat)) return void (0);
                        ...
                        if (esBot.roomSettings.usercommandsEnabled) {
                            API.sendChat('/me [@' + chat.un + '] Usercommands disabled.');
                            esBot.roomSettings.usercommandsEnabled = !esBot.roomSettings.usercommandsEnabled;
                        }
                        else {
                            API.sendChat('/me [@' + chat.un + '] Usercommands enabled.');
                            esBot.roomSettings.usercommandsEnabled = !esBot.roomSettings.usercommandsEnabled;
                        }
                    }
                }
            ...
            voteratioCommand: {
                ...
                functionality: function (chat, cmd) {
                    ...
                    if (!esBot.commands.executable(this.rank, chat)) return void (0);
                        ...
                        var msg = chat.message;
                        if (msg.length === cmd.length) return API.sendChat('[@' + chat.un + '] No user specified.');
                        var name = msg.substring(cmd.length + 2);
                        var user = esBot.userUtilities.lookupUserName(name);
                        if (user === false) return API.sendChat('/me [@' + chat.un + '] Invalid user specified.');
                        var vratio = user.votes;
                        var ratio = vratio.woot / vratio.meh;
                        API.sendChat('/me [@' + chat.un + '] @' + name + ' ~ woots: ' + vratio.woot + ', mehs: ' + vratio.meh + ', ratio (w/m): ' + ratio.toFixed(2) + '.');
                    }
                }
            },
                ...
                functionality: function (chat, cmd) {
                    ...
                    if (!esBot.commands.executable(this.rank, chat)) return void (0);
                        ...
                        if (esBot.roomSettings.welcome) {
                            ...
                            return API.sendChat('/me [@' + chat.un + '] welcome message disabled.');
                        }
                        else {
                            esBot.roomSettings.welcome = !esBot.roomSettings.welcome;
                            return API.sendChat('/me [@' + chat.un + '] welcome message enabled.');
                        }
                    }
                }
            },
                ...
                functionality: function (chat, cmd) {
                    ...
                    if (!esBot.commands.executable(this.rank, chat)) return void (0);
                        ...
                        if (typeof esBot.roomSettings.youtubeLink === "string")
                            API.sendChat('/me [' + chat.un + '] Subscribe to us on youtube: ' + esBot.roomSettings.youtubeLink);
                    }
                }
            }
        }
